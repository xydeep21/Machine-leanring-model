...
pred_bag = predict(bag_boston,data = test_set)
...
pred_rf = predict(rf_boston,data = test_set)
plot(pred_rf,train_set$medv)
mean((pred_rf-train_set$medv)^2)
```
## Using cross validation method to find the best parameter for building Boost Trees
```{r}
...
for (i in 1:length(lambda)){
  ...
  gbm_pred[i] = predict(gbm_boston, data=test_set,n.trees = 5000）
}
gbm_pred
```
```{r}
