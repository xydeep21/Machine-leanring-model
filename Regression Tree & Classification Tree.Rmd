...
dim(Carseats_train)
```
##The tree package will automatically make regression tree or classification tree depends on the target value. If the target value is blooean type it will build classification tree and if the target value is numerical it will build regression tree
```{r}
